...
from src.tools.api_router import (
...
def aswath_damodaran_agent(state: AgentState):
    ...
    Analyze US equities through Aswath Damodaran's intrinsic‑value lens:
      ...
      • Cross‑check with relative valuation (PE vs. Fwd P/E sector median proxy)
...
def analyze_growth_and_reinvestment(metrics: list, line_items: list) -> dict[str, any]:
    ...
    """
    if metrics is None or (hasattr(metrics, 'empty') and metrics.empty) or len(metrics) < 2:
        return {"score": 0, "max_score": 4, "details": "Insufficient history"}
    ...
    return {"score": score, "max_score": 4, "details": "; ".join(details), "metrics": latest.model_dump()}
...
def analyze_risk_profile(metrics: list, line_items: list) -> dict[str, any]:
    ...
    if metrics is None or (hasattr(metrics, 'empty') and metrics.empty) or len(metrics) == 0:
        return {"score": 0, "max_score": 3, "details": "Insufficient data for risk analysis"}
...
def analyze_relative_valuation(metrics: list) -> dict[str, any]:
    ...
    Simple PE check vs. historical median (proxy since sector comps unavailable):
      ...
      ‑1 if >130 %
    """
    if metrics is None or (hasattr(metrics, 'empty') and metrics.empty) or len(metrics) < 5:
        return {"score": 0, "max_score": 1, "details": "Insufficient P/E history"}

    pes = [m.price_to_earnings_ratio for m in metrics if m.price_to_earnings_ratio]
    if len(pes) < 5:
        return {"score": 0, "max_score": 1, "details": "P/E data sparse"}
    ...
    return {"score": score, "max_score": 1, "details": desc}
...
def calculate_intrinsic_value_dcf(metrics: list, line_items: list, risk_analysis: dict) -> dict[str, any]:
    ...
    """
    if metrics is None or (hasattr(metrics, 'empty') and metrics.empty) or len(metrics) < 2 or not line_items:
